...
def get_image(page, direction, bbox, dpi=256):
    pix = doc[pn].get_pixmap(clip=bbox, dpi=dpi, matrix=fitz.Matrix(2,2))
    img = np.frombuffer(buffer=pix.samples_mv, dtype=np.uint8)\
        .reshape((pix.height, pix.width, -1))

    unit = img.shape[1]/bbox.width
    ...
    for i in range(60, mask.shape[0]):
        l = np.sum(mask[i,:])
        if l > min(800, img.shape[1]*0.75):
            img = img[:i,:,:]
            break

    if bbox.x0 < 300 and bbox.width > 400:
        left = round((310 - bbox.x0) * unit)
        right = round((320 - bbox.x0) * unit)
        vl = cv2.cvtColor(img[:,left:right], cv2.COLOR_BGR2GRAY)
        if np.size(vl) == np.sum(vl==255):
            img = img[:,:left]
...
def iter_docs(root):
    for pdf in glob(f"{root}/papers/*.pdf"):
